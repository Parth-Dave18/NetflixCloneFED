...
let getStartedBtns = document.querySelectorAll(".getStarted");
...
langList.forEach((langList) =>
  langList.addEventListener("click", langSelectHandler)
);
...
langList3.forEach((langList3) =>
  langList3.addEventListener("click", langSelectHandler)
);
...
getStartedBtns.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (email.length !== 0 && floatingInput.checkValidity()) {
